...
pub struct EditorConfig {
    ...
    #[field_names(
        desc = "Set the editor line height. 5.0より小さい場合、行の高さはfont sizeの倍数になります。"
    )]
    line_height: f64,
    ...
    pub show_tab: bool,
    #[field_names(desc = "fileに対してnavigation breadcrumbsが表示されるかどうか")]
    pub show_bread_crumbs: bool,
    #[field_names(desc = "editorが最後の行を超えてscrollできるかどうか")]
    pub scroll_beyond_last_line: bool,
    #[field_names(
        desc = "cursorの上下に表示される最小の線数を設定する"
    )]
    pub cursor_surrounding_lines: usize,
    #[field_names(
        desc = "scroll時にeditor上部に関数やclassなどのcode contextを表示"
    )]
    pub sticky_header: bool,
    #[field_names(
        desc = "現在の完了済のitemのdocumentをeditorで表示する場合"
    )]
    pub completion_show_documentation: bool,
    #[field_names(
        desc = "parameterを入力する際に、editorに関数のsignatureを表示させたい場合"
    )]
    pub show_signature: bool,
    #[field_names(
        desc = "signature viewがcodeblockをlabelに入れるべきかどうか。 これは、labelに無効なcodeを提供するLSPではうまく機能しないかもしれません。"
    )]
    pub signature_label_code_block: bool,
    #[field_names(
        desc = "matching pairの自動closeを無効にするかどうか"
    )]
...
pub struct UIConfig {
    ...
    #[field_names(desc = "UIのicon sizeを設定する")]
    icon_size: usize,
